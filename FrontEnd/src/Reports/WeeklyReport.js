...
function Reports() {
  ...
  const groupSalesByDay = () => {
    ...
    weeklySales.forEach((sale) => {
      ...
      const quantity = sale.quantity;
      const totalAmount = unitPrice * quantity;
      const pickupDate = new Date(sale.pickup_time);
      const pickupDayOfWeek = pickupDate.toLocaleString('en-US', { weekday: 'long' });

      if (!groupedSales[pickupDayOfWeek]) {
        groupedSales[pickupDayOfWeek] = {
          sales: [sale],
          totalIncome: totalAmount,
        };
      } else {
        groupedSales[pickupDayOfWeek].sales.push(sale);
        groupedSales[pickupDayOfWeek].totalIncome += totalAmount;
      }
    });
  ...
  const renderGroupedSales = () => {
    ...
    const dayNames = Object.keys(groupedSales);

    return dayNames.map((dayName) => {
      const { sales, totalIncome } = groupedSales[dayName];

      return (
        <React.Fragment key={dayName}>
          <tr style={{ border: '1px solid #ddd', background: '#f2f2f2' }}>
            <td colSpan="6" style={{ padding: '8px', border: '1px solid #ddd' }}>{dayName}</td>
          </tr>
          {sales.map((sale) => {
            const unitPrice = sale.totalprice;
            const quantity = sale.quantity;
          ...
          <tr style={{ border: '1px solid #ddd', background: '#f2f2f2' }}>
            <td colSpan="5" style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'right' }}>Total Income:</td>
            <td style={{ padding: '8px', border: '1px solid #ddd' }}>{totalIncome}</td>
          </tr>
        </React.Fragment>
      );
    });
...
//                 // Get the pickup day of the week for the current sale
//                 const pickupDate = new Date(sale.pickup_time);
//                 const pickupDayOfWeek = pickupDate.toLocaleString('en-US', { weekday: 'long' });
...
//                     <tr style={{ border: '1px solid #ddd', background: '#f2f2f2' }}>
//                       <td colSpan="6" style={{ padding: '8px', border: '1px solid #ddd' }}>{pickupDayOfWeek}</td>
//                     </tr>

//                     {/* Add the regular row for the sale */}
//                     <tr key={sale.orderid} style={{ border: '1px solid #ddd' }}>
//                       <td style={{ padding: '8px', border: '1px solid #ddd' }}>{sale.orderid}</td>
//                       <td style={{ padding: '8px', border: '1px solid #ddd' }}>{`${sale.user.fname} ${sale.user.lname}`}</td>
//                       <td style={{ padding: '8px', border: '1px solid #ddd' }}>{sale.food.food_name}</td>
